...
(function (window, undefined) {
    ...
    var po = window["po"] =
    {
        ...
        dataSorter: function (grid) {
            ...
            this.sort = function (column, dir) {
                var match = ko.utils.arrayFirst(grid.columns(), function (item)
                    { return column == item.columnName });

                if (match.dataType == "Int32") {
                    this.grid.collection.sort(function (current, next) {
                        return dir == "u" ?
                            current[column]() - next[column]() :
                            next[column]() - current[column]();
                    });
                }
                else if (match.dataType == "Date") {
                    this.grid.collection.sort(function (current, next) {
                        return dir == "u" ?
                            new Date(current[column]()).getTime() - new Date(next[column]()).getTime() :
                            new Date(next[column]()).getTime() - new Date(current[column]()).getTime();
                    });
                }
                else { //assume the rest is string type
                    this.grid.collection.sort(function (current, next) {
                        return dir == "u" ?
                            current[column]() > next[column]() ? 1 : -1 :
                            next[column]() > current[column]() ? 1 : -1;
                    });
                }
           };
        }
    ...
    var gridHTML = "\
    <table id=\"poTable\" class=\"table table-bordered table-striped table-hover\" cellspacing=\"0\">\
        <thead data-bind=\"if: headerEnabled()\">\
            <tr data-bind=\"foreach: columns\">\
                <!-- ko if: $data.isVisible -->\
                <th data-bind=\"text: $data.displayName, attr: { poColumn: $data.columnName }, headerBound: $data.columnName, event: {click: $parent.onSort.bind($parent)}\">\
                </th>\
                <!-- /ko -->\
            </tr>\
        </thead>\
        <tfoot data-bind=\"if: showFooterControl\">\
            ...
            <tr data-bind=\"if: pager.enabled()\">\
                <td colspan=\"200\" nowrap=\"nowrap\">\
                    ...
                    <ul>\
                        <li> <button data-bind=\"click: pager.onFirstPage.bind(pager)\" /> << </li>\
                        <li> <button data-bind=\"click: pager.onPrevPage.bind(pager)\" /> < </li>\
                        <li> <button data-bind=\"click: pager.onNextPage.bind(pager)\" /> > </li>\
                        <li> <button data-bind=\"click: pager.onLastPage.bind(pager)\" /> >> </li>\
                        <li> <button> Page <em data-bind=\"text: pager.currentPage()\"></em> of <em data-bind=\"text: pager.totalPageCount()\"></em> </button></li>\
                    </ul>\
                    </div>\
                </td>\
            </tr>\
        ...
        <tbody data-bind=\"foreach: collection.slice(pager.startingRow(), pager.endingRow())\">\
            <tr data-bind=\"foreach: $parent.columns, event: { mouseover: $parent.onMouseIn.bind($parent),  mouseout: $parent.onMouseOut.bind($parent), click: $parent.onClick.bind($parent) }\">\
                <!-- ko if: $data.isVisible -->\
                <td data-bind=\"text: $parent[$data.propertyName], cellBound: $data.propertyName\">\
                </td>\
                <!-- /ko -->\
            </tr>\
        </tbody>\
